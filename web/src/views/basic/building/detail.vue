<template>
    <WaterMark>
        ...
        <Card
            ...
            class="mt-16"
        >
            <Table :columns="owers" :data="detail.owers" />
        </Card>
        ...
        <Drawer :title="owerDrawerTitle" v-model="showOwerDrawer" placement="left" width="70">
            <Table :columns="historyColumn" :data="owerHistoryData" />
            <Spin size="large" fix v-if="owerFetching" />
        </Drawer>
...
export default {
            ...
            owers: [
                {
                    ...
                    render: (h, p) => {
                        if (this.userInfo.access.includes(ROLES.YZDA)) {
                            return h(
                                'a',
                                { on: { click: () => this.$router.push(`/basic/ower/detail/${p.row.user_id}`) } },
                                p.row.real_name
                            );
                        } else {
                        ...
                        if (this.userInfo.access.includes(ROLES.YZDA)) {
                            return h(
                                'a',
                                {
                                    on: {
                                        click: () => {
                                            this.$router.push(`/basic/ower/detail/${p.row.ejyy_wechat_mp_user_id}`);
                                        }
                                    }
                                },
    ...
    methods: {
            ...
            Modal.confirm({
                ...
                onOk: () => {
                    ...
                    utils.request
                        .post(url, data)
                        .then(res => {
                            const owers = [].concat(this.detail.owers);
                            owers[index] = {
                                ...owers[index],
                                status
                            };
                            this.detail.owers = owers;

                            if (index in this.owerHistoryMap) {
                                this.owerHistoryMap[index].unshift({
                                    status,
        ...
        getOwerHistory(info, index) {
            this.owerDrawerTitle = `业主${info.real_name}与${utils.building.text(
                this.detail.info,
                false
            )}绑定关系操作历史`;
            ...
            if (index in this.owerHistoryMap) {
                this.owerHistoryData = this.owerHistoryMap[index];
            } else {
                const data = {
                    id: info.id,
                    community_id: this.postInfo.default_community_id
                ...
                utils.request
                    ...
                    .then(res => {
                        ...
                        this.owerHistoryMap[index] = res.data.list;
                        this.owerHistoryData = res.data.list;
                    })
