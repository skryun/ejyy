    ...
    <WaterMark class="contract-editor">
        <Card dis-hover :bordered="false" title="合同信息">
            ...
            >
                ...
                <FormField title="乙方系业主：" prop="is_ower">
                    <OSwitch v-model="form.is_ower">
                        <span slot="open">否</span>
                        <span slot="close">是</span>
                    </OSwitch>
                </FormField>
                <FormField title="乙方：" prop="second_party" width="300" v-if="!form.is_ower">
                    <Input v-model="form.second_party" placeholder="请输入乙方称呼" />
                </FormField>
                <FormField title="乙方联系人：" prop="second_party_linkman" width="180" v-if="!form.is_ower">
                    <Input v-model="form.second_party_linkman" placeholder="请输入乙方联系人" />
                </FormField>
                <FormField title="乙方联系电话：" prop="second_party_phone" width="220" v-if="!form.is_ower">
                    <Input v-model="form.second_party_phone" placeholder="请输入乙方联系电话" />
                </FormField>
                <FormField title="乙方业主电话：" prop="ower_phone" width="300" v-if="form.is_ower">
                    <Input
                        prefix="ios-contact"
                        class="user-input"
                        :readonly="owerFetching"
                        v-model="form.ower_phone"
                        :suffix="owerFetching ? 'ios-loading' : ''"
                        placeholder="请输入业主手机号码"
                    />
                </FormField>
                <FormField title="乙方业主姓名：" prop="second_party_wechat_mp_user_id" width="300" v-if="form.is_ower">
                    <span v-if="form.second_party_wechat_mp_user_id">{{ owerInfo.real_name }}</span>
                    <span v-else>-</span>
                </FormField>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            form: {
                ...
                ower_phone: detail.ower_phone ? detail.ower_phone : '',
                ...
                is_ower: detail.is_ower ? detail.is_ower : false
            },
            ...
            rules: {
                ...
                ower_phone: [{ required: true, pattern: /^\d{11}$/, message: '请输入正确的业主联系电话' }],
                ...
                is_ower: [{ required: true, type: 'boolean', message: '请选择是否系业主合同' }]
            },
            submiting: false
        };
    },
    ...
    watch: {
        detail: {
            ...
            handler(cur) {
                this.form = {
                    ...
                    ower_phone: cur.ower_phone ? cur.ower_phone : '',
                    ...
                    is_ower: cur.is_ower ? cur.is_ower : false
                };

                this.items = cur.items ? cur.items : [{}];
            }
        },
        'form.ower_phone'(cur, old) {
            if (/^1\d{10}$/.test(cur)) {
                this.owerFetching = true;
                ...
                utils
                    ...
                    .then(res => {
                        this.owerInfo = res.data;
                        this.form.second_party_wechat_mp_user_id = res.data.id;
                        this.owerFetching = false;
                    })
                    .catch(() => (this.owerFetching = false));
            } else {
        ...
        'form.is_ower'(cur) {
            if (!cur) {
                this.owerInfo = {};
                this.form.ower_phone = undefined;
            }
        },
